...
public class Configuration {
    ...
    public Path getTemplateFolderPath() {
        return Paths
                .get( getDefaultParameter( String.class, "template-folder" ) );
    }
    ...
    private ConfigurationData openConfigurationFile( String filename ) {
        try {
            return openJsonFile( ConfigurationData.class,
                                 Host.getInstance()
                                         .getSupportFileReadOnly( filename ) );
        } catch( IOException ex ) {
            Logger.getLogger( Configuration.class.getName() )
                    .log( Level.SEVERE, "cannot open configuration file: " + filename, ex );
        }
        return null;
    }

    private <J> J openJsonFile( Class<J> jsonClass, final File file ) {
        try {
            return new ObjectMapper().readValue( file, jsonClass );
        } catch( IOException ex ) {
